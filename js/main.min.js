...
function resendOtp(e) {
  ...
  axios
    .post(
      `http://api-dcrm-dev.fincity.in/open/opportunity/send-otp?token=${responseData?.token}`
    )
    .then((res) => {
...
function verfiyOtp(e, check) {
  ...
  let otp =
    document.querySelector(
      check == 0 ? "#_1st" : check == 1 ? "#_1st_" : "1st_"
    )?.value +
    document.querySelector(
      check == 0 ? "#_2nd" : check == 1 ? "#_2nd_" : "2nd_"
    )?.value +
    document.querySelector(
      check == 0 ? "#_3rd" : check == 1 ? "#_3rd_" : "#3rd_"
    )?.value +
    document.querySelector(
      check == 0 ? "#_4th" : check == 1 ? "#_4th_" : "#4th_"
    )?.value;
  let body = {
    token: responseData?.token,
    otp: otp,
...
function openApi(name, phone, email) {
  ...
  axios
    .post("https://api-dcrm-dev.fincity.in/open/opportunity", body)
    .then((res) => {
      // if (isOtp) {
      let modalForm = document.querySelector(".enquiryMain");
      let verifyOtp = document.querySelector(".otpVerification");
